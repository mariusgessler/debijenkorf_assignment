...
const SearchBar = () => {
  ...
  <div className="search-bar">
    <div className="search-bar__container">
        <form className={`search-bar__form ${active ? 'active' : 'inactive'}`} onSubmit={handleSubmit} >
          <div className="search-bar__input-container">
            ...
            {input && <button className="search-bar__button search-bar__button--reset" value="Reset">
              <CloseIcon className="search-bar__icon search-bar__icon--reset" title="Reset search" onClick={() => setInput('')}></CloseIcon>
            </button> }
      ...
      <SearchSuggestions query={searchTerm} setSearchTerm={setSearchTerm}/>
  </div>
 );
};
